...
export class ContentAgeRecommendationConsumer extends WorkerHost {
  ...
  async process(job: Job<ContentAgeRecommendationJobPayload, void>) {
    this.logger.log(`Processing age recommendation for video ${job.data.videoId}`);

    if (job.data.ageRecommendation === null) {
      this.logger.log(`Age recommendation is null for video ${job.data.videoId}, skipping`);
      return;
    }
