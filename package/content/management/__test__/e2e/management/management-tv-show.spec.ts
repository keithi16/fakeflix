...
describe('ManagementTvShowController (e2e)', () => {
  ...
  describe('/admin/tv-show (POST)', () => {
    ...
    it('returns 404 when adding an episode to a non-existent tv show', async () => {
      await request(app.getHttpServer())
        .post(`/admin/tv-show/${randomUUID()}/upload-episode`)
        .attach('video', `${CONTENT_TEST_FIXTURES}/sample.mp4`)
        .field('title', 'Test Episode')
        .field('description', 'This is a test episode')
        .field('season', 1)
        .field('number', 1)
