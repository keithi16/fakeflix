...
export class VideoProcessingJobProducer {
  constructor(
    @InjectQueue(QUEUES.VIDEO_AGE_RECOMMENDATION) private videoRecommendationQueue: Queue,
  ...
  async processTranscript(video: Video) {
    this.logger.log(`Queueing video transcript processing job for video ID: ${video.id}`);
  ...
  async processSummary(video: Video) {
    this.logger.log(`Queueing video summary processing job for video ID: ${video.id}`);
